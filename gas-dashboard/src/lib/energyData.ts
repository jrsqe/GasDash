...
function aggregateFacility(unitSeries: Record<string, [string, number][]>) {
...
export async function getEnergyData() {
  ...
  for (const region of REGIONS) {
    ...
    try {
      ...
      const priceSeries = parseTimeseries(priceResp)
      const firstSeries = Object.values(priceSeries)[0] ?? []
      for (const [ts, v] of firstSeries) prices[toAEST(ts)] = v
    } catch (e) {
      console.warn(`Price fetch failed for ${region}:`, e)
    }

    // Fetch generation per facility
    ...
    for (const f of rFacilities) {
      try {
        ...
        if (Object.keys(unitSeries).length > 0) {
          facilitySeriesMap.push({ name: f.name, data: aggregateFacility(unitSeries) })
        }
      } catch (e) {
        console.warn(`Skipping ${f.code}:`, e)
      }
    }
    ...
    const priceVals = Object.values(prices).filter(v => v !== null) as number[]
  ...
  return { ok: true, interval: INTERVAL, data: regionData }
}
